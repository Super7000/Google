...
function saveBookmark() {
    ...
    if (bookmarkNameValue == null || bookmarkNameValue == "") {
        showErrorBox("Please Enter a Bookmark Name");
        return;
    }
    if (bookmarkURLValue == null || bookmarkURLValue == "") {
        showErrorBox("Please Enter URL");
        return;
    }

    if (bookmarkIconPath.files.length != 0)
        createImagePath(bookmarkIconPath.files[0], updateBookmark);
    else
        updateBookmark();

    function updateBookmark(bookmarkIconPathValue = "") {
        // Updating Bookmarks
